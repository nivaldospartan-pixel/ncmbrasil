...
st.markdown(
    ...
    .stButton>button {{
        ...
        color:white; font-weight:bold; border-radius:10px; padding:10px 20px;
    }}
...
def buscar_por_titulo_xml(termo, caminho_xml="GoogleShopping_full.xml", limite=10):
    ...
    try:
        ...
        for item in root.iter():
            ...
            if g_id:
                produtos.append({
                    "SKU": g_id, "Título": titulo, "Link": link,
        ...
        if not produtos:
            return [], "Nenhum produto encontrado no XML."

        descricoes = [p["Título"] for p in produtos]
        escolhas = process.extract(termo, descricoes, scorer=fuzz.WRatio, limit=limite)
        resultados = [produtos[idx] for _, _, idx in escolhas]
...
with tab1:
    ...
    tipo_busca = st.radio("Buscar produto por:", ["SKU", "Título"], horizontal=True)
...
with tab2:
    ...
    if tipo_calc == "SKU":
        sku_calc = st.text_input("Digite o SKU para calcular o IPI:")
