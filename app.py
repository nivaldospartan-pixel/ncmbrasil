...
.stButton>button {background-color:#4B8BBE; color:white; font-weight:bold; border-radius:10px; padding:10px 20px; margin-top:5px;}
...
if st.sidebar.button("Entrar"):
    ...
    if not user_row.empty:
        ...
        df_users.loc[df_users.username==username,"ultimo_acesso"]=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        df_users.to_csv(USERS_FILE,index=False)
    else:
...
with tab1:
    ...
    sku_input = st.text_input("Digite o SKU do produto", key="sku_tab1")
    if sku_input:
...
with tab2:
    ...
    sku_calc = st.text_input("Digite o SKU para calcular o IPI:", key="sku_tab2")
    if sku_calc:
        ...
        if erro: st.error(erro)
            ...
            if st.button("Calcular IPI", key="btn_calc_tab2"):
                try:
